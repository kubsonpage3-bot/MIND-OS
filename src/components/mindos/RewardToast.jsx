...
export function showRewardToast({ xp = 0, gold = 0, boss = 0, streak = 0, label = "", effectNotes = [], isCrit = false, itemDropped = null }) {
  if (_showToast) _showToast({ xp, gold, boss, streak, label, effectNotes, isCrit, itemDropped });
}
...
export default function RewardToast() {
  ...
  useEffect(() => {
    _showToast = ({ xp, gold, boss, streak, label, effectNotes, isCrit, itemDropped }) => {
      const id = Date.now();
      setToasts(prev => [...prev.slice(-3), { id, xp, gold, boss, streak, label, effectNotes, isCrit, itemDropped }]);
        ...
        {toasts.map(t => (
          ...
          >
            <div className="flex items-center gap-3">
              {t.label && (
                <span className="font-game text-xs font-semibold" style={{ color: t.isCrit ? "#fbbf24" : "#f0c040" }}>
