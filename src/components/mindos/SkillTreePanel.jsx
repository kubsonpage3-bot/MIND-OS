...
export default function SkillTreePanel({ skillTree, onUpdate, gold, onSpendGold }) {
  ...
  const unlocked = skillTree.unlockedNodes || [];
  const sp = skillTree.skillPoints || 0;
  ...
  const canUnlock = (node) => {
    ...
    if (gold < node.gold) return false;
    return true;
  };
  ...
  const doRespec = () => {
    if (gold < respecCost) return;
    playSound('error');
    // Refund all SP
    let totalSpRefund = 0;
    Object.values(SKILL_TREE).forEach(branch => {
      branch.nodes.forEach(node => {
        if (unlocked.includes(node.id)) totalSpRefund += node.sp;
      });
    });
    const newTree = { ...skillTree, unlockedNodes: [], skillPoints: sp + totalSpRefund };
    saveRPGData("mindos_skillTree", newTree);
    onUpdate(newTree);
    onSpendGold(respecCost);
    setShowRespec(false);
  };
  ...
  const loadPreset = (preset) => {
    ...
    if (spNeeded > sp || goldNeeded > gold) {
      setConfirmPreset({ preset, spNeeded, goldNeeded, canAfford: false });
    } else {
      setConfirmPreset({ preset, spNeeded, goldNeeded, canAfford: true });
    }
  };

  const applyPreset = (preset, spNeeded, goldNeeded) => {
    const newTree = { ...skillTree, unlockedNodes: preset.nodes, skillPoints: sp - spNeeded };
    saveRPGData("mindos_skillTree", newTree);
    onUpdate(newTree);
    onSpendGold(goldNeeded);
        ...
        {confirmPreset && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              className="rounded-2xl border p-6 max-w-xs w-full space-y-4 text-center"
              style={{ background: "rgba(14,10,8,0.98)", borderColor: "rgba(240,192,64,0.4)" }}
              onClick={e => e.stopPropagation()}
            >
              <div className="font-mono text-sm font-bold" style={{ color: "#f0c040" }}>LOAD: {confirmPreset.preset.name}</div>
              <div className="text-[10px] font-mono text-muted-foreground space-y-1">
                <div>{confirmPreset.preset.nodes.length} nodes</div>
                {confirmPreset.spNeeded > 0 && <div>Needs <span className="text-primary font-bold">{confirmPreset.spNeeded} SP</span></div>}
                {confirmPreset.goldNeeded > 0 && <div>Costs <span className="text-yellow-400 font-bold">{confirmPreset.goldNeeded}G</span></div>}
                {!confirmPreset.canAfford && <div className="text-red-400 font-bold mt-1">Insufficient SP or Gold!</div>}
              </div>
              <div className="flex gap-2">
                <button onClick={() => setConfirmPreset(null)}
                  className="flex-1 py-2 rounded-lg border text-xs font-mono text-muted-foreground border-border/50">
                  CANCEL
                </button>
                <button
                  onClick={() => applyPreset(confirmPreset.preset, confirmPreset.spNeeded, confirmPreset.goldNeeded)}
                  disabled={!confirmPreset.canAfford}
                  className="flex-1 py-2 rounded-lg border text-xs font-mono font-bold disabled:opacity-30"
                  style={{ borderColor: "#f0c04060", color: "#f0c040", background: "rgba(240,192,64,0.1)" }}
                >
                  APPLY
