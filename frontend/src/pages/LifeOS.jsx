        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono text-sm font-bold tracking-wider text-purple-200">
              {t('lifeos.life_os', 'LIFE OS')}
            </span>
            <span className="text-xs text-purple-500/50 hidden sm:inline">
              {t('lifeos.rpg_habit_tracker', 'RPG Habit Tracker')}
            </span>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 rounded-xl border border-purple-800/40 bg-purple-950/30">
        <h3 className="text-purple-300 font-bold text-sm mb-3 uppercase tracking-wider">
          {t('lifeos.equipped', 'Equipped')}
        </h3>
      ...
      <div className="p-4 rounded-xl border border-purple-800/40 bg-purple-950/30">
        <h3 className="text-purple-300 font-bold text-sm mb-3 uppercase tracking-wider">
          {t('lifeos.inventory', 'Inventory')} ({gs.inventory.length})
        </h3>
        {gs.inventory.length === 0 ? (
          <div className="text-purple-500/50 text-xs text-center py-4">
            {t('lifeos.complete_tasks', 'Complete tasks to earn item drops!')}
          </div>
        ) : (
            ...
            {gs.inventory.map(item => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[9px] text-purple-500 mt-0.5 capitalize">
                    {String(t(`lifeos.slots.${item.slot}`, item.slot))}
                  </div>
                  {isEquipped && (
                    <div className="text-[9px] text-yellow-400 mt-0.5">
                      {String(t('lifeos.equipped_badge', 'EQUIPPED'))}
                    </div>
                  )}
                </button>
              );
