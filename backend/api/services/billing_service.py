...
def handle_stripe_webhook(payload: bytes, sig_header: str) -> None:
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if client_reference_id:
            try:
                ...
                logger.error(
                    f"Webhook error: User with ID {client_reference_id} not found."
                )
    ...
    # Handle subscription deletion/cancellation
        ...
        try:
            ...
            profile.save()
            logger.info(
                f"User {profile.user.username} Premium subscription canceled/deleted."
            )
        except UserProfile.DoesNotExist:
            logger.warning(
                f"Webhook error: UserProfile with customer ID {customer_id} not found on subscription.deleted."
            )
