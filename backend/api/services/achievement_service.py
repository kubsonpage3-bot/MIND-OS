...
# The `check` function receives a `UserStats` instance (and profile implicitly via stats.user.profile)  # noqa: E501
ACHIEVEMENTS_SSOT = {
    # Consistency
    "first_step": {
        "gold": 50,
        "sp": 0,
        "check": lambda s: s.total_tasks_completed >= 1,
    },
    ...
    "polymath_ach": {
        ...
        "sp": 0,
        "check": lambda s: (
            len(s.unique_subjects) >= 10
            if isinstance(s.unique_subjects, list)
            else False
        ),
    },
    ...
    "grandmaster": {
        "gold": 5000,
        "sp": 0,
        "check": lambda s: s.highest_subject_rank >= 8,
    },
    # Wealth
    ...
    "warlords_court": {
        "gold": 2000,
        "sp": 0,
        "check": lambda s: s.allies_recruited >= 5,
    },
    ...
    "reborn": {
        "gold": 0,
        "sp": 0,
        "check": lambda s: getattr(s.user, "profile", None)
        and s.user.profile.prestige_count >= 1,
    },
    "phoenix": {
        "gold": 0,
        "sp": 0,
        "check": lambda s: getattr(s.user, "profile", None)
        and s.user.profile.prestige_count >= 3,
    },
}
